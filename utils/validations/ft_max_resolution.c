...
/*   Updated: 2021/10/19 00:15:25 by degabrie         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_max_resolution(t_game *game)
{
	...
	if ((game->plan.height * IMG_SIZE) > height
		|| (game->plan.width * IMG_SIZE) > width)
	{
		printf("Max screen resolution: %dx%d\nMap resolution: %dx%d\n",
			width, height,
			game->plan.width * IMG_SIZE,
			game->plan.height * IMG_SIZE);
